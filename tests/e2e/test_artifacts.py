...
class TestFlashcardsGeneration:
    ...
    async def test_generate_flashcards_default(
        ...
        result = await client.artifacts.generate_flashcards(test_workspace.id)
        assert result is not None
...
class TestInfographicGeneration:
    ...
    async def test_generate_infographic_default(
        ...
        result = await client.artifacts.generate_infographic(test_workspace.id)
        assert result is not None
...
class TestSlideDeckGeneration:
    ...
    async def test_generate_slide_deck_default(
        ...
        result = await client.artifacts.generate_slide_deck(test_workspace.id)
        assert result is not None
...
class TestDataTableGeneration:
    ...
    async def test_generate_data_table_default(
        ...
        result = await client.artifacts.generate_data_table(test_workspace.id)
        assert result is not None
...
class TestArtifactPolling:
    @pytest.mark.asyncio
    @pytest.mark.slow
    async def test_poll_studio_status(self, client, test_workspace):
        result = await client.artifacts.generate_quiz(test_workspace.id)
        assert result is not None
        assert result.task_id, "Quiz generation should return a task_id"

        await asyncio.sleep(2)
        status = await client.artifacts.poll_status(test_workspace.id, result.task_id)
...
class TestMindMapGeneration:
    @pytest.mark.asyncio
    async def test_generate_mind_map(self, client, test_workspace):
        """Mind map generation is fast (~5-10s), not slow."""
        result = await client.artifacts.generate_mind_map(test_workspace.id)
        assert result is not None
...
class TestStudyGuideGeneration:
    ...
    async def test_generate_study_guide(
        ...
        result = await client.artifacts.generate_study_guide(test_workspace.id)
        assert result is not None
